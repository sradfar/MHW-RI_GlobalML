...
#
# This script trains an ExtraTreesClassifier model for predicting rapid intensification 
# (RI) of tropical cyclones based on marine heatwave (MHW) and storm characteristics. It 
# performs grid search to identify the best hyperparameters and logs model performance 
# metrics such as the Probability of Detection (POD) and False Alarm Ratio (FAR).
#
# Outputs:
# - A log file ('model_ET_gs.log') containing the best parameters found, evaluation metrics, 
...
logging.basicConfig(filename='model_ET_gs.log', level=logging.INFO, 
...
def perform_grid_search(X_train, y_train, n_jobs):
    """Performs grid search to find the best parameters for ExtraTreesClassifier."""
...
def main():
    """Main function to run the ExtraTrees model."""
